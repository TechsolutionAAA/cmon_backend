...
exports.getAll = async (req, res) => {
  try {
    ...
    let workingTime =
      workingTimeData.length === 0 ? 0 : workingTimeData[0].count * 7;
    ...
    if (data4Card.length === 0) {
      ...
      const workEndTime =
        data4Card[data4Card.length - 1]._id.TimeStamp.split(" ")[1];
      let overallAction = 0;
      let systemAction = 0;
      ...
      const startH = parseInt(
        data4Card[0]._id.TimeStamp.split(" ")[1].split(":")[0]
      );
      const startM = parseInt(
        data4Card[0]._id.TimeStamp.split(" ")[1].split(":")[1]
      );
      const endH = parseInt(
        data4Card[data4Card.length - 1]._id.TimeStamp.split(" ")[1].split(
          ":"
        )[0]
      );
      const endM = parseInt(
      ...
      for (let i = 0; i <= workingPeriod; i++) {
        label = new Date((startMinutes + i) * 1000 * 60)
          .toISOString()
          .substring(11, 16);
        chartLabels.push(label);
        const overallValue = chartData4OverallEvent.find(
          (t) => t._id.TimeStamp.split(" ")[1] === `${label}`
        )
          ? chartData4OverallEvent.find(
              (t) => t._id.TimeStamp.split(" ")[1] === `${label}`
            ).activityCnt
          : 0;
        const systemValue = chartData4SystemEvent.find(
          (t) => t._id.TimeStamp.split(" ")[1] === `${label}`
        )
          ? chartData4SystemEvent.find(
              (t) => t._id.TimeStamp.split(" ")[1] === `${label}`
            ).activityCnt
          : 0;
        const keyboardValue = chartData4KeyboardEvent.find(
          (t) => t._id.TimeStamp.split(" ")[1] === `${label}`
        )
          ? chartData4KeyboardEvent.find(
              (t) => t._id.TimeStamp.split(" ")[1] === `${label}`
            ).activityCnt
          : 0;
        const mouseValue = chartData4MouseEvent.find(
          (t) => t._id.TimeStamp.split(" ")[1] === `${label}`
        )
          ? chartData4MouseEvent.find(
              (t) => t._id.TimeStamp.split(" ")[1] === `${label}`
            ).activityCnt
          : 0;

        let imageFileName = chartData4Image.find(
          (t) => t.TimeStamp.split(" ")[1] === `${label}`
        )
          ? chartData4Image.find(
              (t) => t.TimeStamp.split(" ")[1] === `${label}`
            ).fileName
          : "";
